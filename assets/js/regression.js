...
class DemingRegression extends Regression {
    ...
    */
    #calculateDeming(x, y) {
        ...
        let b1 =((lambda * q - u) + Math.sqrt((Math.pow(u - lambda * q, 2) + 4 * lambda * Math.pow(p, 2)))) / (2 * lambda * p);
...
class WeightedDemingRegression extends Regression {
    ...
    */
    constructor(errorRatio=DEFAULT_ERROR_RATIO, iterMax = DEFAULT_ITER_MAX, threshold = DEFAULT_THRESHOLD) {
        super();
        this.errorRatio = errorRatio
    ...
    */
    #calculateWeightedDeming(x, y) {
        ...
        let d = new Array(n);
        let x_hat = new Array(n);
        let y_hat = new Array(n);
        let w = new Array(n);
        let w_x = new Array(n);
        let w_y = new Array(n);
        let w_u = new Array(n);
        ...
        while (iter <= this.iterMax) {
            ...
            let b_1 = ((lambda * wq - wu) + Math.sqrt((Math.pow(wu - lambda * wq, 2) + 4 * lambda * Math.pow(wp, 2)))) / (2 * lambda * wp);
            let b_0 = wy - b_1 * wx;
...
class PassingBablokRegression extends Regression {
    ...
    #calculatePaBa(x, y) {
        ...
        let z = jStat.normal.inv(1-this.alpha/2, 0, 1);
        let t = n * (n -1) * (2 * n + 5);
        ...
        for (let j = 0; j < ncols; j++) {
            for (let k=0; k < nrows; k++) {
                if (k < j) {
                    ...
                    if (dx !== 0) { //calculate slope if dx not zero
                        nAllItems += 1;
                        matrix[(j*nrows)+k] = Math.atan(dy/dx);
                    } else if (dy !== 0) { //dx is zero and dy not zero
                        nAllItems += 1;
                        if (this.positiveCorrelated) {
    ...
    #calcDiff(a,  b, eps= 0.000000000001) {
...
class JackknifeConfidenceInterval {
    constructor(x, y, regression, alpha=DEFAULT_ALPHA) {
        this.x = x;
        this.y = y;
    ...
    calculate() {
        ...
        let vx = new Array(size);
        let vy = new Array(size);
        let b_1 = new Array(n);
        let b_0 = new Array(n);
        ...
        let z = jStat.studentt.inv(1-this.alpha/2, size-1);
    ...
    #linnetSE(b_jack, b_global) {
        let n = b_jack.length; //number of data points
        let d_b = new Array(n); //deviations for estimating the standard error
        for (let i = 0; i < n; i++) {
            d_b[i] = (n * b_global) - ((n-1) * b_jack[i]);
        }
        //console.log(`d_b = ${d_b}`, `n = ${n}`);
        return stdev(d_b)/ Math.sqrt(n);
    }
}

class BootstrapConfidenceInterval {
    constructor(x, y, regression, bootstrapN=DEFAULT_BOOTSTRAP_N, alpha=DEFAULT_ALPHA) {
        this.x = x;
        this.y = y;
        this.regression = regression;
    ...
    calculate() {
        ...
        return {
            ...
            slopeLCL: slopeCI[0],
            slopeUCL: slopeCI[1],
            interceptLCL: interceptCI[0],
            interceptUCL: interceptCI[1]
        }
    }

    #calculateBootstrapRegression(x, y, bootstrapN) {
        const b1 = new Array(bootstrapN);
        const b0 = new Array(bootstrapN);
    ...
    #getBootstrapSamples(x, y) {
        const sx = new Array(x.length);
        const sy = new Array(y.length);
...
class MethodCompRegression extends Regression {
    constructor(regressionMethod, errorRatio = DEFAULT_ERROR_RATIO, iterMax = DEFAULT_ITER_MAX, threshold = DEFAULT_THRESHOLD, alpha = DEFAULT_ALPHA, ciMethod = CI_METHOD_DEFAULT, bootstrapN = DEFAULT_BOOTSTRAP_N) {
        super();
...
export { CI_METHOD_BOOTSTRAP, 
    CI_METHOD_JACKKNIFE, CI_METHOD_NONPARAMETRIC, CI_METHOD_DEFAULT, 
    REG_METHOD_DEMING, REG_METHOD_WDEMING, REG_METHOD_PABA,
    DEFAULT_ALPHA, DEFAULT_ERROR_RATIO, DEFAULT_ITER_MAX, DEFAULT_THRESHOLD, 
    DEFAULT_BOOTSTRAP_N,
    DemingRegression, WeightedDemingRegression, PassingBablokRegression, 
    JackknifeConfidenceInterval, BootstrapConfidenceInterval,
    MethodCompRegression, average, stdev, devSq, quantile, };