...
I need help here! This bit might not be right. Seems to work with the balanced 
data set provided in Table 1 of CLSI EP05 but the calculated value of df_WL is
close but not the same when using Analyse-It or VCA (R package) on some other
data sets.

stem:[\alpha_{A}=\frac{1}{b\times r}]

stem:[\alpha_{B}=\frac{r-1}{b\timesr}]

stem:[\alpha_{E}=\frac{r-1}{r}]

stem:[DF_{WL}=\frac{(\alpha_{A}MSA + \alpha_{AB}MSB + \alpha_{E}MSE)^2}{\frac{(alpha_{A}MSA)^2}{DFA} + \frac{(\alpha_{AB}MSB)^2}{DFB} + \frac{(\alpha_{E}MSE)^2}{DFE}}]
