...
const REGEX_RANGE_ONLY =
  "^(?:([a-zA-Z\\d\\x5F\\x2D]*\\x21)?(\\$?[A-Z]{1,3}\\$?[1-9]{1}\\d{0,6})(:\\$?[A-Z]{1,3}\\$?[1-9]{1}\\d{0,6})?)$";

// Optionally match integer or decimal number or as above.
const REGEX_RANGE_PLUS_NUMBERS =
  "^(?:(\\d+\\x2E?\\d*)?(([a-zA-Z\\d\\x5F\\x2D]*\\x21)?(\\$?[A-Z]{1,3}\\$?[1-9]{1}\\d{0,6})(:\\$?[A-Z]{1,3}\\$?[1-9]{1}\\d{0,6})?)?)$";
...
const RangeInput: React.FC<RangeInputProps> = (props: RangeInputProps) => {
  ...
  return (
    <div className={styles.field}>
      <Field label={props.label} validationMessage={validationMessage}>
