    ...
    await Excel.run(async (context) => {
      try {
        ...
        if (inputRange.rowCount < 50) {
          props.notify(
            "warning",
            "This is a small data set and the estimated value of lambda may be inaccurate."
          );
        }
        let data: number[] = [];
        ...
        const lambda = [
          parseFloat(lambdaStartValue),
          parseFloat(lambdaStopValue),
          parseFloat(lambdaStepValue),
        ];
        if (bcMethodValue === "sw") {
        ...
        if (boxCoxOutputCell === "") {
          ...
          const outputRange = boxCoxOutputRange.getAbsoluteResizedRange(
            boxCoxResultsArray.length,
            boxCoxResultsArray[0].length
          );
      ...
      <div className={styles.box}>
        ...
        <div className={styles.grid1x3}>
          <div className={styles.field}>
            ...
            <Input
              type="number"
              value={lambdaStartValue}
              id="lambda-start"
              className={styles.inputfield}
              onChange={handleLambdaStartChange}
            />
          </div>
          ...
          <div className={styles.field}>
            ...
            <Input
              type="number"
              value={lambdaStopValue}
              id="lambda-stop"
              className={styles.inputfield}
              onChange={handleLambdaStopChange}
            />
          </div>
          ...
          <div className={styles.field}>
            ...
            <Input
              type="number"
              value={lambdaStepValue}
              id="lambda-step"
              className={styles.inputfield}
              onChange={handleLambdaStepChange}
            />
          </div>
        </div>
      </div>
      <div className={styles.field}>
        <Button appearance="primary" onClick={boxCoxTransform}>
