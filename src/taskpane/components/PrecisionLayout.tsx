...
export default function PrecisionLayout(props: PrecisionLayoutProps) {
  ...
  const setupPrecision = async () => {
    await Excel.run(async (context) => {
      try {
        ...
        const valuesRange = range
          .getCell(1, 2)
          .getAbsoluteResizedRange(layout.length - 1, numLevels);
  ...
  return (
    <div className="container">
      <div>
        Use this section to setup a layout in Excel to assess an assays imprecision. It can be used
        for both verification and validation studies.
      </div>
      ...
      <div>
        <RangeInput
          label={props.uitext["lbl_layout_range"]}
      ...
      <div className={styles.field}>
        <Button appearance="primary" onClick={setupPrecision}>
