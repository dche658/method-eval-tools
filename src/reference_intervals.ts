...
function non_parametric(data: number[], alpha: number = 0.05, confAlpha = 0.1): ReferenceInterval {
  ...
  if (confAlpha <= 0.05 && n < 153) {
    throw new Error(
      "At least 153 subjects are required to calculate 95% confidence intervals using a non-parametric method."
    );
  }
  if (confAlpha <= 0.05 && n < 120) {
    throw new Error(
      "At least 120 subjects are required to calculate 90% confidence intervals using a non-parametric method."
    );
  }
 ...
 * @param q reference interval quantile, usually 0.025 or 0.975
 * @param alpha level of significance, 0.1 for 90% confidence interval
 * @returns rank
 */
 ...
 * 
 * Background information on the log-likelihood function for a Box-Cox transformation
 * can be found here.
 * 
 * https://www.css.cornell.edu/faculty/dgr2/_static/files/R_html/Transformations.html
 * 
 * The algorithm generated by ChatGPT is a simplification that works for univariate 
 * data. It has removed a number of constants so the calculated log likelihood will 
 * not be exact but the relative values will be proportionate.
 * 
 * @param data - original positive data
 * @param lambda - transformation parameter
